...
#'
#' @description Update prior probabilities of models/hypotheses to posterior
#' probabilities using Bayes factors.
#'
#' @param bf A numeric vector of non-negative values.
#' @param prior_prob A numeric vector with values in the \[0,1\] interval.
#' If `length(bf) == 1` then `prior_prob` can be of any positive
#' \code{\link[base]{length}}, but if `length(bf) > 1` then the
#' \code{\link[base]{length}} of `prior_prob` can only be `1`
#' or equal to the \code{\link[base]{length}} of `bf`.
#'
#' @details `bfactor_to_prob` turns Bayes factors into posterior
#' probabilities using a vectorized version of the following
#' equation from \insertCite{bergerDelampady1987;textual}{pcal}:
#'
#' \deqn{
#'       P(H_0 | x) = \left(
#'         1 + \frac{1 - \pi_0}{\pi_0} \, \frac{1}{B_{01}(x)}
#'         \right)^{-1}
#'       }{
#'       P(H0 | x) = (1 + ( (1 - \pi) / \pi) (1 / B(x) ) ) ^ ( -1 )
#' }
#' where \eqn{B_{01}(x)}{B(x)} is a Bayes factor in favor of the null
#' hypothesis (given the data \eqn{x}), \eqn{\pi_0}{\pi} is the prior
#' probability of the null hypothesis and \eqn{1 - \pi_0}{1 - \pi}
...
#' be of \code{\link[base]{length}} equal to `length(bf)`, in which case
#' each prior probability in `prior_prob` will be updated using the
#' corresponding element of `bf`, or of \code{\link[base]{length}} `1`,
#' in which case it will be recycled (if `length(bf) > 1`) and each
#' element of `bf` will update the same `prior_prob` value.
#'
#' @return If `length(bf) > 1` then `bfactor_to_prob` returns a numeric
#' vector with the same \code{\link[base]{length}} as `bf`, otherwise it
#' returns a numeric vector with the same \code{\link[base]{length}} as
...
#'
#' @seealso
#' * \code{\link[pcal]{bfactor_interpret}} for the interpretation of
#'   Bayes factors.
#'
#' @examples
...
bfactor_to_prob <- function(bf, prior_prob = .5) {
  ...
  if (isTRUE(
    length(bf) > 1) &&
    isFALSE(length(prior_prob) %in% c(1, length(bf)))) {
    stop(
      "Invalid argument:
       if length(bf) > 1 then length(prior_prob) can only be 1
       or equal to length(bf)."
       )
  }
