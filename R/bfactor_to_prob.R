...
#'
#' @description Update prior probabilities of models/hypotheses to posterior probabilities using Bayes factors.
#'
...
#'
#' @details `bfactor_to_prob` turns Bayes factors into posterior probabilities using a vectorized version of the following equation from \insertCite{bergerDelampady1987;textual}{pcal}:
#'
#' \deqn{P(H_0 | x) = \left(1 + \frac{1 - \pi_0}{\pi_0} \, \frac{1}{B_{01}(x)} \right)^{-1}}{P(H0 | x) = (1 + ( (1 - \pi) / \pi) (1 / B(x) ) ) ^ ( -1 )}
#'
#' where \eqn{B_{01}(x)}{B(x)} is a Bayes factor in favor of the null hypothesis (given the data \eqn{x}), \eqn{\pi_0}{\pi} is the prior probability of the null hypothesis and \eqn{1 - \pi_0}{1 - \pi} is the prior probability of the alternative hypothesis.
#'
#' If `bf` is a vector of Bayes factors (in favor of the null hypothesis) and `prior_prob` is a vector with the prior probabilities of those same hypotheses then `bfactor_to_prob(bf, prior_prob)` updates `prior_prob` to posterior probabilities. The posterior probabilities of the alternative hypotheses can be obtained with  `1 - bfactor_to_prob(bf, prior_prob)`.
#'
