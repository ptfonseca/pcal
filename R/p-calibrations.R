...
#' @inheritParams bcal
#'
#' @details \insertCite{sellke2001;textual}{pcal} developed the following calibration of p-values into lower bounds for the posterior probabilities of point null hypotheses (or lower bounds for the probabilities of type I errors): \mjdeqn{\alpha(p) = (1 + \lbrack -e \,  p \operatorname{log}(p) \rbrack^{-1})^{-1}}{alpha(p) = (1 + \[-e p log(p)\]^(-1))^(-1)} where \mjeqn{p}{p} is a  p-value on a classical test statistic. This calibration assumes that both the null and the alternative hypotheses have 0.5 prior probability. `pcal` generalizes the aforementioned calibration for prior probabilities other than 0.5: \mjdeqn{\alpha(p) = \left(1 + \frac{1 - \pi_0}{\pi_0} \, \lbrack -e \,  p \operatorname{log}(p) \rbrack^{-1}\right)^{-1}}{alpha(p) = (1 + (1 - pi_0) / pi_0 \[ -e p log(p) \]^(-1))^(-1)} where \mjeqn{\pi_0}{pi_0} is the prior probability of the null hypothesis. Note that `pcal(p, prior_prob)` is equivalent to \code{\link[pcal]{bfactor_to_prob}(\link[pcal]{bcal}(p), prior_prob)}.
#'
