...
#' @inheritParams bcal
#'
#' @details `pcal` is a vectorized implementation of the calibration of p-values into lower bounds for the posterior probabilities of point null hypotheses (or lower bounds for the probabilities of type I errors) developed by \insertCite{sellke2001;textual}{pcal}. The calibration is: \deqn{\alpha(p) = (1 + \lbrack -e \,  p log(p) \rbrack^{-1})^{-1}}{\alpha(p) = 1 / ( 1 + ( - e p log(p) ) ^ ( -1 ) )} where \eqn{p} is a  p-value on a classical test statistic. This calibration assumes that both the null and the alternative hypotheses have 0.5 prior probability. We generalized the aforementioned calibration for prior probabilities other than 0.5: \deqn{\alpha(p) = \left(1 + \frac{1 - \pi_0}{\pi_0} \, \lbrack -e \,  p log(p) \rbrack^{-1}\right)^{-1}}{\alpha(p) = 1 / ( 1 + ( ( 1 - \pi ) / \pi ) ( - e  p log(p) ) ^ ( -1 ) )} where \eqn{\pi_0}{\pi} is the prior probability of the null hypothesis and \eqn{1 - \pi_0}{1 - \pi} is the prior probability of the alternative hypothesis.
#'
...
#'
#'  Note that `pcal(p, prior_prob)` is equivalent to \code{\link[pcal]{bfactor_to_prob}(\link[pcal]{bcal}(p), prior_prob)}.
#'
